  ...
  <div>
    ...
    <div class="table-wapper">
      ...
      >
        ...
        <el-table-column label="最低人数">
          <template slot-scope="scope">
            <span>{{ scope.row.minPerson }}</span>
          </template>
        </el-table-column>
        <el-table-column label="最多人数">
          <template slot-scope="scope">
            <span>{{ scope.row.maxPerson }}</span>
          </template>
        </el-table-column>
    ...
    <el-dialog :title="dialogTitle" :visible.sync="dialogFormVisible" width="500px">
      <el-form :model="formData" :rules="rules" ref="ruleForm" label-width="100px">
        <el-form-item label="课程" prop="courseId">
          <el-select v-model.trim="formData.courseId" clearable>
        ...
        <el-form-item label="分层名称" prop="courseLayerName">
          <el-input v-model.trim="formData.courseLayerName"></el-input>
        </el-form-item>
        <el-form-item label="课时(节/周)" prop="weekHours">
          <el-input-number v-model="formData.weekHours" :min="1" :max="20"></el-input-number>
        </el-form-item>
        <el-form-item label="最低人数" prop="minPerson">
          <el-input v-model.number="formData.minPerson" clearable></el-input>
        </el-form-item>
        <el-form-item label="最多人数" prop="maxPerson">
          <el-input v-model.number="formData.maxPerson" clearable></el-input>
        </el-form-item>
      </el-form>
  ...
  methods: {
    ...
    saveBtn() {
      this.$refs['ruleForm'].validate(async valid => {
        if (valid) {
          const { minPerson, maxPerson } = this.formData
          if (minPerson >= maxPerson) {
            this.$message.error('最低人数必须小于最多人数')
            return
          }
          const params = {}
