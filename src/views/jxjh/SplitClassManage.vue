      ...
      <el-table ref="multipleTable" :data="tableData" tooltip-effect="dark" highlight-current-row style="width: 100%" :height="height">
        ...
        <el-table-column label="教学班" property="classNames">
          <template slot-scope="scope">
            <el-tag size="medium" v-for="(item,index) in scope.row.classNames" :key="index">{{ item.name }}</el-tag>
          </template>
        </el-table-column>
        <el-table-column label="预计人数" property="expectNum"></el-table-column>
        <el-table-column label="任课教师" property="teachers">
          <template slot-scope="scope">
            <div v-for="(item,index) in scope.row.teachers" :key="index"><el-input v-model="item.name"/></div>
          </template>
        </el-table-column>
...
<script>
import { getChooseClassListInfo } from '@/api/pkcx' // getSbjestClassListInfo:学生分层课时数据
export default {
  data() {
    return {
      // 检索字段
